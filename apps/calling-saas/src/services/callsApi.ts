...
function getAuthHeader(): Record<string, string> {
  const token = localStorage.getItem('token') || localStorage.getItem('enr_token') || localStorage.getItem(CALL_TOKEN_KEY);
...
async function request<T = any>(
  ...
  if (!res.ok) {
    ...
    throw new Error(message || `The calling request failed (${res.status}).`);
  }
...
export const callsApi = {
  ...
  importContactsCsv: async (file: File) => {
    ...
    return res.json() as Promise<{ imported: number; duplicates?: number; contacts: Contact[] }>;
  },
  ...
  updateContact: (id: number, data: { notes?: string | null; meeting_time?: string | null; next_call_at?: string | null; stage?: string; omnichannel_stage?: string; deal_value?: number; lost_reason?: string; current_crm?: string | null; pain_points?: string | null; automation_opportunities?: string | null }) =>
