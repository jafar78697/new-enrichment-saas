...
export interface UseWebRTCDeviceResult {
...
function toLinear16(input: Float32Array): ArrayBuffer {
...
export function useWebRTCDevice(agentId: number | null | undefined): UseWebRTCDeviceResult {
  ...
  const [incomingCall, setIncomingCall] = useState<UseWebRTCDeviceResult['incomingCall']>(null);
  ...
  const handleCallNotification = useCallback((notification: any) => {
    ...
    console.info('[CallingService] Telephony notification', notification);
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition && !recognitionRef.current) {
      ...
      recognition.onresult = (event: any) => {
        if ((window as any).isAiSpeakingRightNow) {
           // Suppress local microphone transcription if the AI is currently speaking out of the speakers (Echo cancellation)
           return;
        }
        ...
        setTranscriptMessages(prev => {
           ...
           if (lastYouIndex >= 0 && !existing[lastYouIndex].isFinal) {
              const msg = existing[lastYouIndex];
              msg.text = text;
              msg.isFinal = isFinal;
              // If it's not already at the end, move it to the end so it stays chronological
              if (lastYouIndex !== existing.length - 1) {
                 existing.splice(lastYouIndex, 1);
                 existing.push(msg);
              }
           } else {
  ...
  const startTranscription = useCallback(() => {
    ...
    try {
      ...
      socket.on('connect', () => {
        ...
        socket.emit('start', { callId, sampleRate: context.sampleRate });
      });
      ...
      socket.on('transcript', (payload: { text?: string; isFinal?: boolean }) => {
        ...
        setTranscriptMessages(prev => {
          ...
          if (lastClientIndex >= 0 && !existing[lastClientIndex].isFinal) {
             const msg = existing[lastClientIndex];
             msg.text = text;
             msg.isFinal = Boolean(payload.isFinal);
             // If it's not already at the end, move it to the end so it stays chronological
             if (lastClientIndex !== existing.length - 1) {
                existing.splice(lastClientIndex, 1);
                existing.push(msg);
             }
          } else {
      ...
      processor.onaudioprocess = (event) => {
        ...
        socket.emit('audio', toLinear16(samples));
        ...
        if (rms > 0.01) {
           (window as any).isAiSpeakingRightNow = true;
           if (aiSpeakingTimeout) window.clearTimeout(aiSpeakingTimeout);
           aiSpeakingTimeout = window.setTimeout(() => {
               (window as any).isAiSpeakingRightNow = false;
           }, 500);
        }
      };
...
export default useWebRTCDevice;
