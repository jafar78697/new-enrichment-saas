...
export class OutreachImapService {
    ...
    try {
      ...
      for (const item of messages) {
        ...
        const { rowCount } = await this.db.query(
          `SELECT id FROM unified_inbox WHERE tenant_id = $1 AND msg_id = $2`,
          [account.tenant_id, msgId]
        );
        ...
        if (inReplyTo) {
          const { rows: matchByHeaders } = await this.db.query(
            `SELECT lead_id, campaign_id, id FROM outreach_logs 
             WHERE tenant_id = $1 AND msg_id = $2 LIMIT 1`,
             [account.tenant_id, inReplyTo]
          );
        ...
        if (!leadId && fromEmail) {
          const { rows: matchByEmail } = await this.db.query(
            `SELECT ol.lead_id, ol.campaign_id, ol.id
        ...
        if (leadId) {
          await this.db.query(
            `UPDATE outreach_logs 
             ...
             [account.tenant_id, leadId, campaignId]
          );
