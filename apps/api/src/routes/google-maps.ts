...
import { verifyToken } from '../calls-module/middleware/auth.js';
  ...
  fastify.post('/v1/google-maps/scrape', {
    preValidation: async (request: any, reply: any) => {
      try {
        ...
        verifyToken(token); // Validates the HS256 token from calls-module
      } catch (err: any) {
    ...
    try {
      const { keywords, location, limit = 60, niche_name } = request.body as { keywords: string[]; location?: string; limit?: number; niche_name?: string };
