...
async function pollResult(db: any, jobId: string, timeoutMs = 55000): Promise<any> {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const { rows: jobRows } = await db.query(
      `SELECT status FROM enrichment_jobs WHERE id = $1`, [jobId]
    );
    if (jobRows[0]?.status === 'failed' || jobRows[0]?.status === 'cancelled') return null;
...
export default async function publicEnrichRoutes(fastify: FastifyInstance) {

  // POST /v1/public/enrich — single domain enrichment
  fastify.post('/v1/public/enrich', async (request: any, reply) => {
    ...
    if (tenantRows[0]) {
      ...
      const { rows: newTenant } = await fastify.db.query(
        `INSERT INTO tenants (name, slug, plan) VALUES ('Internal', 'internal', 'pro')
    ...
    const { rows: jobRows } = await fastify.db.query(
      `INSERT INTO enrichment_jobs (tenant_id, source_type, mode, status, total_items)
    ...
    // Create job item
    const { rows: itemRows } = await fastify.db.query(
      `INSERT INTO enrichment_job_items (job_id, tenant_id, raw_input, normalized_domain, shard_index)
       VALUES ($1, $2, $3, $4, 0)
       RETURNING id`,
      [jobId, tenantId, domain, normalized]
    );
  ...
  fastify.get('/v1/public/result/:job_id', async (request: any, reply) => {
    const apiKey = request.headers['x-api-key'];
    if (apiKey !== PUBLIC_API_KEY) {
      return reply.code(401).send({ error: 'Invalid API key' });
    }
    ...
    const { rows } = await fastify.db.query(
      `SELECT r.* FROM enrichment_results r
       JOIN enrichment_job_items ji ON ji.id = r.job_item_id
       WHERE ji.job_id = $1 LIMIT 1`,
      [request.params.job_id]
    );

    if (!rows[0]) {
      const { rows: jobRows } = await fastify.db.query(
        `SELECT status FROM enrichment_jobs WHERE id = $1`, [request.params.job_id]
      );
