...
export default async function passwordResetRoutes(app: FastifyInstance) {
  ...
  app.post('/v1/auth/forgot-password', async (req, reply) => {
    ...
    if (!rows[0]) return { message: 'If this email exists, a reset link has been sent.' };
  ...
  app.post('/v1/auth/reset-password', async (req, reply) => {
    ...
    if (!token || !password) return reply.code(422).send({ error: 'token and password required' });
