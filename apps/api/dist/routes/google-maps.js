...
async function googleMapsRoutes(fastify) {
    fastify.post('/v1/google-maps/scrape', {
        ...
        let meteredJobId = null;
        let enrichmentJobId = null;
        const walletService = new wallet_service_1.WalletService(db);
        try {
            const { keywords: submittedKeywords, location, limit, limitPerKeyword } = request.body;
            ...
            if (Array.isArray(submittedKeywords)) {
                submittedKeywords.forEach((value) => {
                    ...
                    if (keyword)
                        keywordLookup.set(keyword.toLowerCase(), keyword);
                });
            }
            const keywords = [...keywordLookup.values()];
            const resultsPerKeyword = Math.max(1, Math.min(Number(limitPerKeyword ?? limit) || MAX_LEADS_PER_KEYWORD, MAX_LEADS_PER_KEYWORD));
            ...
            const initClient = await db.connect?.() || db;
            const usesInitTransaction = typeof db.connect === 'function';
            try {
                if (usesInitTransaction)
                    await initClient.query('BEGIN');
                const { rows: tenantRows } = await initClient.query(`SELECT plan FROM tenants WHERE id = $1 FOR UPDATE`, [tenantId]);
                if (tenantRows[0]?.plan === 'demo') {
                    const { rows: usageRows } = await initClient.query(`SELECT COALESCE(SUM(cardinality(keywords)), 0)::int AS used
             ...
             WHERE tenant_id = $1 AND status IN ('queued', 'running', 'completed')`, [tenantId]);
                    ...
                    if (keywordsUsed + keywords.length > DEMO_KEYWORD_LIMIT) {
                        const error = new Error(`Your free demo includes ${DEMO_KEYWORD_LIMIT} keyword searches. Upgrade your account to continue.`);
                ...
                const { rows: meteredRows } = await initClient.query(`INSERT INTO metered_maps_jobs (tenant_id, user_id, keywords, location, max_credits, status, reservation_id, started_at)
           VALUES ($1, $2, $3, $4, $5, 'running', $6, NOW())
           RETURNING id`, [tenantId, userId || null, keywords, requestedLocation, maxResults, reservationId]);
                meteredJobId = meteredRows[0].id;
                const { rows: jobRows } = await initClient.query(`INSERT INTO enrichment_jobs (tenant_id, workspace_id, status, source_type, mode)
           VALUES ($1, $2, 'processing', 'google-maps', 'maps_scrape') RETURNING id`, [tenantId, workspaceId]);
            ...
            for (const keyword of keywords) {
                ...
                while (leadsForKeyword < resultsPerKeyword && allLeads.length < maxResults) {
                    ...
                    for (const lead of leads) {
                        ...
                        if (allowedLeads.length >= Math.min(remainingForKeyword, remainingForBatch))
                            break;
                    }
            ...
            const finishClient = await db.connect?.() || db;
            const usesFinishTransaction = typeof db.connect === 'function';
            try {
                if (usesFinishTransaction)
                ...
                if (allLeads.length > 0) {
                    const jobItemValues = [];
                    const jobItemPlaceholders = [];
                    const meteredValues = [];
                    const meteredPlaceholders = [];
                    let idx = 0;
                    for (const lead of allLeads) {
                        const jobItemOffset = idx * 5;
                        const leadInput = lead.website || lead.name || lead.place_id || `google-maps-${idx}`;
                        jobItemValues.push(enrichmentJobId, tenantId, leadInput, leadInput, idx);
                        ...
                        meteredValues.push(meteredJobId, tenantId, lead.place_id || null, lead.name, lead.address, lead.phone, lead.website || null, lead.category || null, JSON.stringify(lead));
                    ...
                    const { rows: jobItems } = await finishClient.query(`INSERT INTO enrichment_job_items (
               job_id, tenant_id, raw_input, normalized_domain, status, shard_index, started_at, finished_at
             ) VALUES ${jobItemPlaceholders.join(', ')}
             RETURNING id, shard_index`, jobItemValues);
                    const jobItemIds = new Map(jobItems.map((item) => [Number(item.shard_index), item.id]));
                    const enrichmentValues = [];
                    const enrichmentPlaceholders = [];
                    for (let resultIndex = 0; resultIndex < allLeads.length; resultIndex++) {
                        ...
                        if (!jobItemId)
                            throw new Error('Could not create an enrichment job item for a Maps lead.');
                        const rawLead = {
                            ...lead,
                            source: 'google_maps',
                        ...
                        enrichmentValues.push(jobItemId, tenantId, lead.website || lead.name, lead.phone, lead.name, JSON.stringify(rawLead));
                    ...
                    await finishClient.query(`INSERT INTO enrichment_results (job_item_id, tenant_id, domain, primary_phone, company_name, raw_data)
             VALUES ${enrichmentPlaceholders.join(', ')}`, enrichmentValues);
                    ...
                    if (contactLeads.length > 0) {
                        ...
                        contactLeads.forEach((lead, contactIndex) => {
                            ...
                            contactValues.push(tenantId, businessName, String(lead.phone).trim(), businessName, lead.website || null, 'google_maps');
                        ...
                        const { rows: existingContacts } = await finishClient.query(`SELECT phone_number
               FROM contacts
               WHERE tenant_id = $1 AND phone_number = ANY($2::TEXT[])`, [tenantId, contactLeads.map((lead) => String(lead.phone).trim())]);
                        existingLeadCount = existingContacts.length;
                        newLeadCount = Math.max(0, contactLeads.length - existingLeadCount);
                        await finishClient.query(`UPDATE contacts AS contact
               ...
               WHERE contact.tenant_id = incoming.tenant_id
                 AND contact.phone_number = incoming.phone_number`, contactValues);
                        await finishClient.query(`INSERT INTO contacts (tenant_id, name, phone_number, company, website, source)
               ...
               WHERE NOT EXISTS (
                 ...
                 WHERE contact.tenant_id = incoming.tenant_id
                   AND contact.phone_number = incoming.phone_number
                    ...
                    await finishClient.query(`INSERT INTO metered_maps_results (job_id, tenant_id, place_id, business_name, address, phone, website, rating, review_count, category, raw_data)
             VALUES ${meteredPlaceholders.join(', ')}`, meteredValues);
                }
                await finishClient.query(`UPDATE enrichment_jobs SET status = 'completed', total_items = $1, completed_items = $1 WHERE id = $2`, [allLeads.length, enrichmentJobId]);
                await finishClient.query(`UPDATE metered_maps_jobs
           SET status = 'completed', credits_used = $1, results_count = $1, completed_at = NOW()
           WHERE id = $2`, [allLeads.length, meteredJobId]);
                if (reservationId) {
            ...
            if (reservationId) {
                try {
                    await walletService.release(reservationId);
                }
                catch (e) { }
            }
            if (meteredJobId) {
                try {
                    await db.query(`UPDATE metered_maps_jobs SET status = 'failed', error_message = $1, completed_at = NOW() WHERE id = $2`, [err.message || 'Maps job failed', meteredJobId]);
                }
                catch { }
            }
            if (enrichmentJobId) {
