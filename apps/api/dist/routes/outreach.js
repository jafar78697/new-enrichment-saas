...
async function outreachRoutes(fastify) {
    ...
    */
    ...
    fastify.get('/v1/outreach/facebook/tasks', { preHandler: [authenticateApiKey] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { rows } = await fastify.db.query(`
    ...
    fastify.post('/v1/outreach/facebook/sync-reply', { preHandler: [authenticateApiKey] }, async (request, reply) => {
        const { tenantId } = request.tenant;
    ...
    fastify.get('/v1/outreach/meta/profiles', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        ...
        try {
            const { rows } = await fastify.db.query(`SELECT * FROM meta_profiles ${whereClause}`, params);
    ...
    fastify.post('/v1/outreach/meta/profiles', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { name, platform, username, profile_url } = request.body;
        if (!name)
    ...
    fastify.delete('/v1/outreach/meta/profiles/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
        await fastify.db.query(`DELETE FROM meta_profiles WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
        return reply.send({ success: true });
    });
    // --- META DAILY STATS ---
    // GET /v1/outreach/meta/daily-stats?profile_id=&date=
    fastify.get('/v1/outreach/meta/daily-stats', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, date } = request.query;
        const targetDate = date || new Date().toISOString().split('T')[0];
        try {
            const where = ['tenant_id = $1', 'date = $2'];
            const params = [tenantId, targetDate];
            if (profile_id) {
                params.push(profile_id);
                where.push(`profile_id = $${params.length}`);
            }
            const { rows } = await fastify.db.query(`SELECT action_type, SUM(count)::int as count FROM meta_daily_actions 
         WHERE ${where.join(' AND ')} GROUP BY action_type`, params);
    ...
    fastify.post('/v1/outreach/meta/actions/log', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, action_type, count } = request.body;
    ...
    fastify.get('/v1/outreach/meta/pipeline', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, platform } = request.query;
        try {
            ...
            const { rows } = await fastify.db.query(`SELECT * FROM meta_pipeline WHERE ${where.join(' AND ')} ORDER BY stage_updated_at DESC`, params);
            // Group by stage
            const pipeline = {
    ...
    fastify.post('/v1/outreach/meta/pipeline', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, name, source_group, platform, avatar_url, stage } = request.body;
        if (!name)
    ...
    fastify.patch('/v1/outreach/meta/pipeline/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
        const { stage, last_message, engagement_level } = request.body;
        const updates = ['stage_updated_at = now()'];
        const params = [];
        if (stage) {
            params.push(stage);
        ...
        if (last_message !== undefined) {
            params.push(last_message);
            updates.push(`last_message = $${params.length}`);
        }
        if (engagement_level) {
            params.push(engagement_level);
            updates.push(`engagement_level = $${params.length}`);
        }
        if (params.length === 0)
            return reply.code(400).send({ error: 'Nothing to update' });
        params.push(id);
        params.push(tenantId);
        const { rows } = await fastify.db.query(`UPDATE meta_pipeline SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND tenant_id = $${params.length} RETURNING *`, params);
        return reply.send({ lead: rows[0] });
    });
    // DELETE /v1/outreach/meta/pipeline/:id
    fastify.delete('/v1/outreach/meta/pipeline/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
    ...
    fastify.get('/v1/outreach/meta/campaigns', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { platform } = request.query;
        try {
            const where = ['tenant_id = $1'];
            ...
            const { rows } = await fastify.db.query(`SELECT * FROM meta_campaigns WHERE ${where.join(' AND ')} ORDER BY created_at DESC`, params);
    ...
    fastify.post('/v1/outreach/meta/campaigns', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { name, platform, message_template, daily_limit, target_groups, profile_id } = request.body;
    ...
    fastify.patch('/v1/outreach/meta/campaigns/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
        const { status, name, message_template, daily_limit } = request.body;
        const updates = [];
        const params = [];
        if (status) {
            params.push(status);
        ...
        if (message_template) {
            ...
            updates.push(`message_template = $${params.length}`);
        }
        if (daily_limit) {
            params.push(daily_limit);
            updates.push(`daily_limit = $${params.length}`);
        }
        if (!updates.length)
            return reply.code(400).send({ error: 'Nothing to update' });
        params.push(id);
        params.push(tenantId);
        const { rows } = await fastify.db.query(`UPDATE meta_campaigns SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND tenant_id = $${params.length} RETURNING *`, params);
        return reply.send({ campaign: rows[0] });
    });
    // DELETE /v1/outreach/meta/campaigns/:id
    fastify.delete('/v1/outreach/meta/campaigns/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
    ...
    fastify.get('/v1/outreach/meta/queue', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, status } = request.query;
        try {
    ...
    fastify.post('/v1/outreach/meta/queue', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { profile_id, pipeline_id, task_type, target_url, message_body } = request.body;
    ...
    fastify.patch('/v1/outreach/meta/queue/:id', { preHandler: [authenticateApiKey] }, async (request, reply) => {
        const { tenantId } = request.tenant;
        const { id } = request.params;
    ...
    fastify.post('/v1/outreach/meta/actions/sync-progress', async (request, reply) => {
        ...
        const { profileEmail, dailyProgress, weeklyProgress, date } = request.body;
        ...
        try {
            ...
            let profileRes = await fastify.db.query(`SELECT id FROM meta_profiles WHERE tenant_id = $1 AND username = $2`, [tenantId, profileEmail]);
            let profileId;
            if (profileRes.rows.length === 0) {
                // Auto-create profile if not exists
                const newProfile = await fastify.db.query(`INSERT INTO meta_profiles (tenant_id, name, username, platform) VALUES ($1, $2, $3, $4) RETURNING id`, [tenantId, profileEmail.split('@')[0], profileEmail, 'facebook']);
    ...
    fastify.post('/v1/outreach/meta/launch-profiles', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        try {
