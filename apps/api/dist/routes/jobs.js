...
function dedupeAndNormalize(domains) {
    const seen = new Set();
    const result = [];
...
async function checkHttpQuota(db, tenantId, plan, count) {
    const { rows } = await db.query(`SELECT http_enrichments_used, http_limit FROM usage_counters
     WHERE tenant_id = $1 AND billing_period_start = date_trunc('month', now())::date`, [tenantId]);
    if (!rows[0])
        return true;
...
async function jobRoutes(fastify) {
    ...
    fastify.post('/v1/jobs/enrich', {
        ...
        const db = new db_1.JobRepository(fastify.db, tenantId);
        const job = await db.create({
            workspace_id: workspaceId, created_by: userId, source_type: 'api',
            mode: body.mode || contracts_1.EnrichmentMode.SMART_HYBRID,
            total_items: uniqueDomains.length,
            idempotency_key: body.idempotency_key,
            webhook_url: body.webhook_url
        });
        if (!job)
            return reply.code(400).send({ error: 'DUPLICATE_JOB', message: 'Duplicate idempotency key' });
    ...
    fastify.post('/v1/jobs/enrich-csv', {
        ...
        if (websiteIdx === -1)
            return reply.code(422).send({ error: 'No website/domain column found in CSV' });
        const rawDomains = lines.slice(1).map((l) => l.split(',')[websiteIdx]?.trim()).filter(Boolean);
        const uniqueDomains = dedupeAndNormalize(rawDomains);
        if (uniqueDomains.length > MAX_DOMAINS)
            return reply.code(422).send({ error: 'JOB_TOO_LARGE', message: `Max ${MAX_DOMAINS} domains per job` });
        const withinQuota = await checkHttpQuota(fastify.db, tenantId, plan, uniqueDomains.length);
        if (!withinQuota)
            return reply.code(402).send({ error: 'QUOTA_EXCEEDED', message: 'Monthly HTTP enrichment limit reached.' });
        const db = new db_1.JobRepository(fastify.db, tenantId);
        const job = await db.create({ workspace_id: workspaceId, created_by: userId, source_type: 'csv', mode: contracts_1.EnrichmentMode.SMART_HYBRID, total_items: uniqueDomains.length });
        if (!job)
    ...
    fastify.get('/v1/jobs', {
        ...
        const { rows } = await fastify.db.query(`SELECT * FROM enrichment_jobs WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, [tenantId, limit, offset]);
    ...
    fastify.get('/v1/jobs/:id', {
        ...
        const { tenantId } = request.tenant;
        const { rows } = await fastify.db.query(`SELECT * FROM enrichment_jobs WHERE id = $1 AND tenant_id = $2`, [request.params.id, tenantId]);
        if (!rows[0])
    ...
    fastify.get('/v1/jobs/:id/results', {
        ...
        const offset = (page - 1) * limit;
        const conditions = ['r.tenant_id = $1', 'ji.job_id = $2'];
        const params = [tenantId, request.params.id];
        if (q.has_email === 'true')
            conditions.push('r.primary_email IS NOT NULL');
        if (q.has_phone === 'true')
            conditions.push('r.primary_phone IS NOT NULL');
        if (q.has_linkedin === 'true')
            conditions.push('r.linkedin_url IS NOT NULL');
        if (q.confidence === 'high')
            conditions.push("r.confidence_level = 'high_confidence'");
        if (q.lane === 'browser')
            conditions.push("r.enrichment_lane = 'browser'");
        if (q.failed === 'true')
            conditions.push("ji.status = 'failed'");
       ...
       WHERE ${where} ORDER BY r.created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, limit, offset]);
        const { rows: countRows } = await fastify.db.query(`SELECT COUNT(*) FROM enrichment_results r JOIN enrichment_job_items ji ON ji.id = r.job_item_id WHERE ${where}`, params);
    ...
    fastify.post('/v1/jobs/:id/cancel', {
        ...
        const { tenantId } = request.tenant;
        await fastify.db.query(`UPDATE enrichment_jobs SET status = 'cancelled', finished_at = now() WHERE id = $1 AND tenant_id = $2`, [request.params.id, tenantId]);
        await fastify.db.query(`UPDATE enrichment_job_items SET status = 'failed' WHERE job_id = $1 AND status = 'queued'`, [request.params.id]);
        return { success: true };
    });
    ...
    fastify.post('/v1/jobs/:id/retry-failed', {
        ...
        const { rows: failedItems } = await fastify.db.query(`SELECT ji.* FROM enrichment_job_items ji
       JOIN enrichment_jobs j ON j.id = ji.job_id
       WHERE ji.job_id = $1 AND j.tenant_id = $2 AND ji.status = 'failed'`, [request.params.id, tenantId]);
        if (!failedItems.length)
            return { retried: 0 };
        const isPriority = plan === 'pro';
        for (const item of failedItems) {
            await fastify.db.query(`UPDATE enrichment_job_items SET status = 'queued', http_attempts = 0 WHERE id = $1`, [item.id]);
            await queue_1.producer.sendToHttpQueue({ job_item_id: item.id, job_id: item.job_id, tenant_id: tenantId, domain: item.normalized_domain, mode: contracts_1.EnrichmentMode.SMART_HYBRID, attempt: 1, enqueued_at: new Date().toISOString() }, isPriority);
        }
    ...
    fastify.post('/v1/jobs/:id/export', {
        ...
        const { rows } = await fastify.db.query(`INSERT INTO exports (tenant_id, job_id, format, status) VALUES ($1, $2, $3, 'pending') RETURNING id`, [tenantId, request.params.id, format]);
        await queue_1.producer.sendToExportQueue({ export_id: rows[0].id, job_id: request.params.id, tenant_id: tenantId, format });
    ...
    fastify.post('/v1/enrich/domain', {
        ...
        const { domain, mode = contracts_1.EnrichmentMode.SMART_HYBRID } = request.body || {};
        const { tenantId, userId, workspaceId, plan } = request.tenant;
        if (!domain)
