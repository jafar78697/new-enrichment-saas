...
async function crmRoutes(fastify) {
    ...
    fastify.patch('/v1/leads/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        const updates = [];
        const params = [];
        const { rows: current } = await fastify.db.query(`SELECT lead_stage, lead_owner_id FROM enrichment_results WHERE id = $1 AND tenant_id = $2`, [request.params.id, tenantId]);
        if (!current[0])
            return reply.code(404).send({ error: 'Lead not found' });
