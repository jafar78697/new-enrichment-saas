...
async function lsRequest(path, method = 'GET', body) {
    ...
    if (!res.ok)
        throw new Error(`LemonSqueezy error: ${res.status}`);
    return res.json();
}
async function billingRoutes(fastify) {
    ...
    fastify.get('/v1/billing/usage', {
        ...
        const { rows } = await fastify.db.query(`SELECT * FROM usage_counters WHERE tenant_id = $1 ORDER BY billing_period_start DESC LIMIT 1`, [tenantId]);
    ...
    fastify.post('/v1/billing/checkout', {
        ...
        const { plan, credits } = request.body;
        let variantId;
    ...
    fastify.post('/v1/webhooks/lemonsqueezy', async (request, reply) => {
        ...
        // Verify signature
        const hmac = crypto_1.default.createHmac('sha256', secret).update(rawBody).digest('hex');
        if (hmac !== signature)
        ...
        if (eventName === 'order_created') {
            ...
            if (credits) {
                // Add browser credits
                await fastify.db.query(`UPDATE usage_counters SET browser_credits_remaining = browser_credits_remaining + $1
           WHERE tenant_id = $2 AND billing_period_start = date_trunc('month', now())::date`, [parseInt(credits), tenantId]);
            }
            if (plan) {
                // Upgrade plan
                await fastify.db.query(`UPDATE tenants SET plan = $1 WHERE id = $2`, [plan, tenantId]);
                await fastify.db.query(`UPDATE usage_counters SET http_limit = $1, browser_credits_remaining = $2
           WHERE tenant_id = $3 AND billing_period_start = date_trunc('month', now())::date`, [PLAN_HTTP_LIMITS[plan], PLAN_BROWSER_CREDITS[plan], tenantId]);
            }
        }
