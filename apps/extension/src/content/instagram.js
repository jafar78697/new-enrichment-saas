...
function setupReplyTracker() {
  if (!window.location.href.includes('/direct/t/')) return;

  // Run initial sync
  setTimeout(syncReplies, 5000);

  // Setup observer for chat switches
...
function syncReplies() {
  try {
    ...
    // On Instagram, chat messages are often inside div elements with specific roles.
    const messageRows = document.querySelectorAll('div[dir="auto"]');
    if (messageRows.length === 0) return;

    // Get the last text block in the chat
...
function triggerConfetti(element) {
  ...
  element.style.background = '#fde047'; 
...
function setupActionTracking() {
  document.addEventListener('click', (e) => {
    // Basic heuristic for Instagram Like button
    const likeSvg = e.target.closest('svg[aria-label="Like"]');
    if (likeSvg) {
    ...
    // Instagram submit comment button
    const submitBtn = e.target.closest('div[role="button"][tabindex="0"]');
  ...
  // Listen for Enter key on comment inputs in IG
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      const el = e.target;
      if (el.tagName === 'TEXTAREA' || el.getAttribute('aria-label')?.includes('comment')) {
        chrome.runtime.sendMessage({ type: 'RECORD_ACTION', action: 'comment' });
        const bar = document.getElementById('jento-comments-bar');
        if (bar) triggerConfetti(bar);
      }
